...
const AdminDashboard = ({ data, setKeySelected , theme }) => {
  ...
  return (
    <div  className="body" style={{padding:'0 5%'}}>
      <div className="title">
        <h1 style={{paddingLeft:'2%' , fontWeight:'bold' ,fontSize:'25px' , color: theme === 'light' ? 'black' : 'white'}}>DASHBOARD</h1>
        <div style={{paddingLeft:'2%' ,color: theme === 'light' ? 'grey' : 'white' , fontSize:'17px' ,fontWeight:'500'}}>Welcome <span style={{color: 'blue ' ,fontWeight:'bold'}}>{name} ,</span> everything looks great.</div>
      </div>
              ...
              <div className="top">
                ...
                <div className="right">
                  <div style={{ borderRadius: '25px', backgroundColor: '#F6F7F9', padding: '16px' }}>
                    ...
                    {item === "comments" && <CommentOutlined style={{ fontSize: '29px', color: 'blue' }} />}
                  </div>
                </div>
              </div>
              
              <div className="bot" onClick={() => setKeySelected(item)}>
                {item === "users" && <span style={{ color: 'blue' }} >View all users</span>}
                {item === "products" && <span style={{ color: 'blue' }} >View all products</span>}
                {item === "orders" && <span style={{ color: 'blue' }} >View all orders</span>}
                {item === "comments" && <span style={{ color: 'blue' }} >View all comments</span>}
